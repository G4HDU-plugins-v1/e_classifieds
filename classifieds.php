...
if (file_exists(THEME . 'eclassifieds_template.php')) {
    define(ECLASSF_THEME, THEME . 'eclassifieds_template.php');
} else {
    define(ECLASSF_THEME, e_PLUGIN . 'e_classifieds/templates/eclassifieds_template.php');
}
...
if (e_QUERY) {
    ...
    $eclassf_from = (int)varset($eclassf_tmp[0], 0);
    $eclassf_action = varset($eclassf_tmp[1], 'cat');
    $eclassf_catid = (int)varset($eclassf_tmp[2], 0);
    $eclassf_subid = (int)varset($eclassf_tmp[3], 0);
    $eclassf_itemid = (int)varset($eclassf_tmp[4], 0);
    $eclassf_tmp = (int)varset($eclassf_tmp[5], 0);
} else {
    $eclassf_from = (int)varset($_REQUEST['eclassf_from'], 0);
    $eclassf_action = varset($_REQUEST['eclassf_action'], 'cat');
    $eclassf_catid = (int)varset($_REQUEST['eclassf_catid'], 0);
    // $eclassf_subid = intval(varset($_REQUEST['eclassf_subid'], 0));
    foreach($_REQUEST['eclassf_subid'] as $row) {
        if ($row > 0 && $eclassf_subid == 0)
            $eclassf_subid = $row;
    }
    $eclassf_itemid = (int)varset($_REQUEST['eclassf_itemid'], 0);
    $eclassf_tmp = (int)varset($_REQUEST['eclassf_tmp'], 0);
}
...
$ECLASSF_PREF['eclassf_perpage'] = (int)varset($ECLASSF_PREF['eclassf_perpage'], 10);
if ($ECLASSF_PREF['eclassf_perpage'] < 5) $ECLASSF_PREF['eclassf_perpage'] = 5;
...
switch ($eclassf_action) {
    ...
    case 'list':
        ...
        $eclassf_arg = '
				...
				limit ' . $eclassf_from . ',' . (int)$ECLASSF_PREF['eclassf_perpage'];
        $eclassf_carg = '
	select count(*) as numrecs from #eclassf_ads as r
	left join #eclassf_subcats as t on r.eclassf_category=t.eclassf_subid
	left join #eclassf_cats as c on t.eclassf_categoryid=c.eclassf_catid
	where r.eclassf_category="' . $eclassf_subid . '" and find_in_set(c.eclassf_catclass,"' . USERCLASS_LIST . '") ' . $eclassf_doapproved . ' and (eclassf_expires = 0 or eclassf_expires>' . $eclassf_today . ')';

        $eclassf_count = $sql->db_Select_gen($eclassf_carg, false);
        extract($sql->db_Fetch());
        $eclassf_count = $numrecs;
        // print $eclassf_count;
        ...
        $eclassf_npaction = $eclassf_obj->make_url($eclassf_from,"list",$eclassf_catid,$eclassf_subid,0);
        // print $ECLASSF_PREF['eclassf_perpage'];
        $eclassf_npparms = $eclassf_count . "," . $ECLASSF_PREF['eclassf_perpage'] . ','. $eclassf_npaction;
    ...
    case 'item':
        $ip = $e107->getip();
        // update unique views on IP address
        $sql->db_Insert("eclassf_unique", "{$eclassf_itemid},'{$ip}'", false);
        // update views
        $sql->db_Update('eclassf_ads', 'eclassf_views=eclassf_views+1 where eclassf_id=' . $eclassf_itemid . '', false);

        $eclassf_arg = 'select eclassf_catname,eclassf_subname
		from #eclassf_cats
		left join #eclassf_subcats on eclassf_catid=eclassf_categoryid
		where eclassf_subid="' . $eclassf_subid . '"';
        $sql->db_Select_gen($eclassf_arg,false);
        ...
        $eclassf_arg = '
				select *,
				count(un.eclassf_unique_adid) as unique_views,
				u.user_id,u.user_name from #eclassf_ads as r
				...
				left join #eclassf_unique as un on eclassf_unique_adid=eclassf_id
        ...
        if ($sql->db_Select_gen($eclassf_arg, false)) {
            ...
            // print $eclassf_prefix;
            $eclassf_fprefix = '\b' . $eclassf_prefix . "." ;

            $eclassf_list = $eclassf_file->get_files('images/classifieds/' , $eclassf_fprefix, 'standard', 0, 0);
            $eclassf_count = 0;
            ...
            if ($eclassf_numrecs < 2 || $eclassf_gallery == 0) {
                foreach($eclassf_list as $eclassf_piclist) {
                    ...
                    if (!empty($eclassf_bigpic) && file_exists('./images/classifieds/' . $eclassf_bigpic)) {
                        ...
                        $eclassf_itempicture .= "
					<a href='" . $url . "&amp;eclassf_height={$ECLASSF_PREF['eclassf_pich']}' rel='lightbox.mcgroup' ><img src='" . $url . "&amp;eclassf_height={$ECLASSF_PREF['eclassf_thumbheight']}&amp;eclassf_watermark={$ECLASSF_PREF['eclassf_watermark']}' title='' style='border:0px' alt=''/></a>";
                    } else {
...
";
            ...
            if ($ECLASSF_PREF['eclassf_userating'] && !$eclassf_left) {
                ...
                $eclassf_namerate = "<a href='../../user.php?id.$eclassf_id' >" . $tp->toHTML($eclassf_postername) . "</a>&nbsp;{$eclassf_view_rating}&nbsp;";
            } elseif ($eclassf_left) {
            ...
            if ($ECLASSF_PREF['eclassf_notifymethod'] == 0 || $ECLASSF_PREF['eclassf_notifymethod'] == 2) {
                $eclassf_jemailaddr .= '
			<script type="text/javascript">
			<!--
            ...
            // $eclassf_itemviews = $eclassf_views . " (Unique {$unique_views})";
            if ($ECLASSF_PREF['eclassf_counter'] == 'NONE') {
                $eclassf_itemviews = '';
            } elseif ($ECLASSF_PREF['eclassf_counter'] == 'ng' || $eclassf_counter == 'ng') {
                $eclassf_itemviews = $eclassf_views . " (Unique {$unique_views})";
            } else {
                $eclassf_itemviews = $eclassf_views . " (Unique {$unique_views})";
                if ($ECLASSF_PREF['eclassf_counter'] == 'ALL' && !empty($eclassf_counter)) {
                    // user defined counter and one is set
                    $eclassf_itemviews = eclassf_makeimg($eclassf_views, $eclassf_counter) . ' (unique ' . eclassf_makeimg($unique_views, $eclassf_counter) . ')' ;
                }
                if ($ECLASSF_PREF['eclassf_counter'] == 'ALL' && !empty($eclassf_counter)) {
                    // user defined counter and one is set
                    $eclassf_itemviews = eclassf_makeimg($eclassf_views, $eclassf_counter) . ' (unique ' . eclassf_makeimg($unique_views, $eclassf_counter) . ')';
                ...
                if ($ECLASSF_PREF['eclassf_counter'] != 'ALL' && $ECLASSF_PREF['eclassf_counter'] != 'NONE') {
                    ...
                    $eclassf_itemviews = eclassf_makeimg($eclassf_views, $ECLASSF_PREF['eclassf_counter']) . ' (unique ' . eclassf_makeimg($unique_views, $ECLASSF_PREF['eclassf_counter']) . ')';
                }
            }
            ...
            if ($ECLASSF_PREF['eclassf_subdrop'] == 1) {
                ...
                if ($sql->db_Select_gen($eclassf_arg, false)) {
                    while ($eclassf_row = $sql->db_Fetch()) {
                        ...
                        // $eclassf_count = $sql2->db_Count("eclassf_ads", "(*)", "where eclassf_category=$eclassf_subid $eclassf_approved and (mlcassf_posted = 0 or mlcassf_posted='' or mlcassf_posted is null or mlcassf_posted>$eclassf_today) ");
                    ...
                    if ($sql2->db_Select_gen($eclassf_selarg, false)) {
                        ...
                        while ($eclassf_subs = $sql2->db_Fetch()) {
                            ...
                            if ($ECLASSF_PREF['eclassf_subdrop'] == 1) {
                                ...
                                $eclassf_url = $eclassf_obj->make_url($eclassf_from, 'list', $eclassf_catid, $eclassf_subid, $eclassf_itemid);
                                ...
                                $catsubs .= "<a href='" . $eclassf_url . "' >" . $tp->toHTML($eclassf_subname, false) . " ($eclassf_count)</a><br />";
                            }
                        }
