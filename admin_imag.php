...
$sql->db_Select("eclassf_ads", "eclassf_prefix", "order by eclassf_id", "nowhere", false);
while ($eclassf_rows = $sql->db_Fetch()) {
    $eclassf_listid[] = $eclassf_rows['eclassf_prefix'];
} // while
...
$eclassf_fprefix = "";
$eclassf_list = $eclassf_file->get_files($dir, $eclassf_fprefix, "standard", 1);
$eclassf_count = 0;
...
if ($eclassf_numrecs > 0) {
    foreach($eclassf_list as $eclassf_row => $key) {
        ...
        if ($fred[0] == 'Thumbs.db' || $fred[0] == 'index.htm' ||  in_array($fred[0].'_', $eclassf_listid)) {
        } else {
            $eclassf_img++;
            $eclassf_text .= "
		<tr>
...
$ns->tablerender(ECLASSF_A1, $eclassf_text,'eclassf_imag');
